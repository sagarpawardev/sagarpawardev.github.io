...
exports.default = $stylesheet.create(
  ...
  "",
  7,
  /*! ./components/VideoPlayer/VideoPlayer.st.css */ 301
);
...
exports.default.$skin.$render = function render_css($id, $params, $functions) {
  return "\n" + $id + ".Video2224969105--root{position: relative}\n" + $id + " .Video2224969105--playerContainer{position: relative}\n" + $id + " .Video2224969105--cover{position: absolute;top: 0;left: 0;height: 100%;width: 100%;background: #000 50% 50% no-repeat;background-size: cover;cursor: pointer}\n" + $id + " .Video2224969105--overlay{display: flex;height: 100%;justify-content: center;flex-direction: column;align-items: center;background-color: rgba(0,0,0,0.35)}\n" + $id + " .Video2224969105--title{margin: 0 15px 15px;text-align: center}\n" + $id + ".VideoPlayer94256039--root{box-shadow: " + $params["boxShadow"] + ";border: " + $params["borderSize"] + " solid " + $params["borderColor"] + ";margin-top: calc(-1 * " + $params["borderSize"] + ");margin-left: calc(-1 * " + $params["borderSize"] + ")}\n" + $id + ".VideoPlayer94256039--root .Video2224969105--overlay{background-color: " + $params["backgroundColor"] + "}\n" + $id + ".VideoPlayer94256039--root .Video2224969105--title{font: " + $params["font"] + ";color: " + $params["fontColor"] + ";margin: 0 0 15px;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;overflow: hidden;max-height: 2.8em}\n" + $id + ".VideoPlayer94256039--root[data-videoplayer94256039-ismobileview] .Video2224969105--title{font-size: calc(18 * " + $params["textSizeScale"] + ");margin: 0 0 10px}\n" + $id + ".VideoPlayer94256039--root .Video2224969105--overlay{padding: 0 10%}\n" + $id + ".VideoPlayer94256039--root .VideoPlayer94256039--playButton{display: block;width: " + $params["playButtonSize"] + ";height: " + $params["playButtonSize"] + ";padding: 0;border: 0;background: transparent;box-sizing: border-box;cursor: pointer}\n" + $id + ".VideoPlayer94256039--root[data-videoplayer94256039-ismobileview] .VideoPlayer94256039--playButton{width: " + $params["playButtonMobileSize"] + ";height: " + $params["playButtonMobileSize"] + "}\n" + $id + " .VideoPlayer94256039--playButton circle{stroke: " + $params["fontColor"] + "}\n" + $id + " .VideoPlayer94256039--playButton path{fill: " + $params["fontColor"] + "}\n";
};
...
var _VideoPlayer = __webpack_require__(/*! ./VideoPlayer */ 553);

var _VideoPlayerSt = __webpack_require__(/*! ./VideoPlayer.st.css */ 301);
...
exports.default = $stylesheet.create(
  "root",
  "Video2224969105",
  {"root":"Video2224969105--root","playerContainer":"Video2224969105--playerContainer","cover":"Video2224969105--cover","overlay":"Video2224969105--overlay","title":"Video2224969105--title"},
  "",
  1,
  /*! ../node_modules/wix-ui-core/dist/src/components/Video/Video.st.css */ 46
);
...
var _Video = __webpack_require__(/*! wix-ui-core/Video */ 554);

var _events = __webpack_require__(/*! ./bi/events */ 558);

var _videoPlayerBehaviors = __webpack_require__(/*! ./behaviors/videoPlayerBehaviors */ 559);

var _VideoPlayerSt = __webpack_require__(/*! ./VideoPlayer.st.css */ 301);

var _VideoPlayerSt2 = _interopRequireDefault(_VideoPlayerSt);

var _playButtonIcon = __webpack_require__(/*! ./playButtonIcon */ 560);
...
var VideoPlayer = exports.VideoPlayer = function VideoPlayer(santaProps) {
  var styleState = {
    ...
    isTitleExists: Boolean(santaProps.title)
  };

  return _react2.default.createElement(_Video.Video, _extends({
    src: santaProps.src,
    title: santaProps.title || '',
    loop: santaProps.loop,
    playing: santaProps.playing,
    muted: santaProps.muted,
    logoUrl: santaProps.logoUrl,
    poster: santaProps.poster,
    playableRef: santaProps.playableRef,
    onPlay: santaProps.onPlay,
    onPause: santaProps.onPause,
    onEnd: santaProps.onEnd,
    controls: santaProps.controls,
    fillAllSpace: true,
    preload: 'none',
  ...
  }, (0, _VideoPlayerSt2.default)('root', styleState, santaProps)));
};
...
VideoPlayer.propTypes = {
  ...
  title: (0, _propTypes.oneOfType)([_propTypes.bool, _propTypes.string]),
  loop: _propTypes.bool,
  playing: _propTypes.bool,
  muted: _propTypes.bool,
  logoUrl: _propTypes.string,
  poster: _propTypes.string,
  playableRef: _propTypes.func,
  onPlay: _propTypes.func,
  onPause: _propTypes.func,
  onEnd: _propTypes.func,
  onMouseIn: _propTypes.func,
...
var santaVideoPlayer = exports.santaVideoPlayer = (0, _createReactClass2.default)({
  ...
  componentDidMount: function componentDidMount() {
    ...
    this.playable.on('video-events/current-time-updated', function (currentTime) {
      _this.handleProgress(currentTime);
    });
    this.playable.on('video-events/volume-changed', function (volume) {
      _this.setState({ volume: volume });
    });
    this.playable.on('video-events/mute-changed', function (isMuted) {
      _this.setState({ isMuted: isMuted });
    });
    this.playable.on('engine-state/metadata-loaded', function () {
      var duration = _this.playable.getDurationTime();
      var volume = _this.playable.getVolume();
      var isMuted = _this.playable.getMute();
      ...
      _this.handleAction(ACTION_TYPES.ITEM_READY);
    });

    listenToEventOnce(this.playable, 'engine-state/play-requested', this.handleFirstPlayRequested);
    listenToEventOnce(this.playable, 'engine-state/playing', this.handleFirstPlay);
    listenToEventOnce(this.playable, 'engine-state/ended', this.handleFirstEnd);
  },
  ...
  UNSAFE_componentWillReceiveProps: function UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.isPlayingAllowed && !this.props.isPlayingAllowed && this.state.isReady) {
      this.handleAction(ACTION_TYPES.ITEM_READY);
    }
  },
  play: function play(callback) {
    listenToEventOnce(this.playable, 'engine-state/playing', callback);
    this.playable.play();
  },
  pause: function pause(callback) {
    listenToEventOnce(this.playable, 'engine-state/paused', callback);
    this.playable.pause();
  },
  stop: function stop(callback) {
    listenToEventOnce(this.playable, 'video-events/reset-playback', callback);
    this.playable.reset();
  },
  togglePlay: function togglePlay(callback) {
    var eventName = this.playable.isVideoPaused ? 'engine-state/playing' : 'engine-state/paused';
    listenToEventOnce(this.playable, eventName, callback);
    this.playable.togglePlayback();
  },
  mute: function mute(callback) {
    listenToEventOnce(this.playable, 'video-events/mute-changed', callback);
    this.playable.setMute(true);
  },
  unmute: function unmute(callback) {
    listenToEventOnce(this.playable, 'video-events/mute-changed', callback);
    this.playable.setMute(false);
  },
  seek: function seek(time, callback) {
    var timePoint = Number(time);
    if (_lodash2.default.isFinite(timePoint)) {
      this.playable.goTo(timePoint);

      listenToEventOnce(this.playable, 'video-events/current-time-updated', callback);
    }
  },
  setVolume: function setVolume(volume) {
    this.playable.setVolume(Number(volume));
  },
  ...
  handleFirstEnd: function handleFirstEnd() {
  ...
  getPosterUrl: function getPosterUrl() {
    ...
    var previewData = imageClientApi.getData(imageClientApi.fittingTypes.SCALE_TO_FILL, src, target);
  ...
  getLogoUrl: function getLogoUrl() {
    ...
    var previewData = imageClientApi.getData(imageClientApi.fittingTypes.SCALE_TO_FIT, src, target, LOGO_TRANSFORM_TYPE);
  ...
  getSkinProperties: function getSkinProperties() {
    ...
    var title = showVideoTitle ? videoTitle : false;
    ...
    var props = {
      ...
      playableRef: function playableRef(playable) {
        _this2.playable = playable;
      },
...
module.exports = __webpack_require__(/*! ./dist/src/components/Video */ 555);
...
Object.defineProperty(exports, "__esModule", { value: true });
var Video_1 = __webpack_require__(/*! ./Video */ 556);
...
var React = __webpack_require__(/*! react */ 0);
var playable_1 = __webpack_require__(/*! playable */ 557);
var Video_st_css_1 = __webpack_require__(/*! ./Video.st.css */ 46);
